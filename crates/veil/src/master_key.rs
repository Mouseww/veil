...
/// Load the 32-byte master key. `VEIL_MASTER_KEY` (then `VEIL_MASTER_KEY`) wins over `master.key`.
///
/// Never log the returned key.
pub fn load_or_create(data_dir: &Path, mode: Mode) -> Result<[u8; 32], Error> {
    if let Ok(val) = crate::env::var("MASTER_KEY") {
