...
跟 AI 聊天，字会离开这台电脑。密码、电话、数据库地址会跟着走。

**Veil 是挂在门口的纱帘。** 机密出门先换成化名；回答进门，化名再变回真名。你还是看原文；窗外那台模型从头到尾只看见化名。

[English](README.md) · [下载](https://github.com/Mouseww/veil/releases/latest) · Apache-2.0 · 无遥测

任何能改 Base URL 的工具都能从这道帘子走出去：Claude Code、Cursor、Codex、Trae、自己写的脚本、公司中转——不限名单。
...
| | 屋里（你） | 化名（占位符） | 窗外（模型） |
|---|---|---|---|
| 出网 | `mysql://root:Pass123@10.1.2.3/app`，`13800138000` | `{{CONNSTR_…}}` `{{PHONE_…}}` | 能顺着化名推理，看不见真身 |
| 进门 | 回答里已经是真连接串、真手机号 | 同一化名整场对话都指同一个人 | 它一直以为自己在讨论化名 |

同一段密码，整场对话都用同一个化名，模型不会把一个人认成两个。

Key 不用交给 Veil：登录和 API Key 还在原来的软件里。Veil 只给**正文**起化名，证件（鉴权头）原样出门。

```
你的工具  --改个地址-->  本机纱帘（化名 / 还原）  --再出门-->  官方 / 中转 / LiteLLM
```
