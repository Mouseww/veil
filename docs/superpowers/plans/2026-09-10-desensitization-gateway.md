...
**Goal:** Ship a local/internal AI desensitization reverse proxy (`dgw`) that redacts secrets in Anthropic Messages, OpenAI Chat Completions, and OpenAI Responses (including SSE restore), plus a management UI and a Claude Code plugin — without changing downstream request parameters.
...
- Restore only when Creator matches. Miss/expired/other-creator → emit token, `restore_miss`, do not abort. Abort only if the store cannot be queried and a valid placeholder appears.
...
- Fail-closed A1. Request body default 32 MiB, hard cap 128 MiB → 413.
...
- [ ] **Step 4: `cargo test -p dgw-engine creator` — PASS**
...
(If the timeout test is flaky on the evil regex, substitute a matcher that sleeps in tests via `Matcher::FailOpenTimeout` test hook — production still uses thread timeout. Prefer a deterministic test hook: `#[cfg(test)] Matcher::BlockUntilTimeout`.)
...
- Exact allowlist contains → skip that exact substring occurrence.
...
1. PEM / AKIA / `sk-ant-` / `sk-` high-entropy / `Bearer eyJ` → `AKSK` (200)
2. Connection URI and ADO-style `Pwd=` strings → `CONNSTR` (180)
3. China ID (17 digits + check) → `IDCARD` (150)
4. China mobile → `PHONE` (140)
...
`MemoryStore` is introduced here as an in-memory `MappingStore` in `mapping.rs` if Task 7 is not yet done — implement a minimal in-memory store in this task (see Step 3) so walk tests run.
...
- `restore_json` finds `PLACEHOLDER_REGEX` in strings; `lookup` Hit → substitute plaintext (do not scan again); Miss → leave token; `Unavailable` → `Err(WalkError::StoreUnavailable)`.
...
Cases:
- `default_data_dir()` on Windows ends with `\\dgw` and uses local app data (mock via `DGW_DATA_DIR` in tests — tests must set `DGW_DATA_DIR` and assert override wins).
- Missing key file + no env in `Mode::Desktop` → writes 32 random bytes hex to `master.key` (mode 0600 when OS supports).
- `Mode::Server` + missing key + missing env → `Err(NoMasterKey)`.
...
- `try_acquire` when pid live → `AlreadyRunning { proxy, management }` success-equivalent for CLI start.
...
- [ ] **Step 3: Implement JSON logs to `data_dir/logs/dgw.log` with rotation (5 MiB × 3). Fields allowed: ts, level, protocol, method, path_template, status, upstream_ms, streaming, hit_types, hit_counts, creator_prefix8, error_class. Never log headers, bodies, tokens.
...
1. `GET /v1/models` empty body → forwarded, no mapping writes.
2. JSON POST with no hits → forwarded, body equal.
3. JSON POST with phone → upstream body contains `{{PHONE_` and not the digits; mapping row exists.
4. Body 32 MiB+1 → 413, mock upstream receives 0 requests.
5. Outgoing request to mock has no `accept-encoding` gzip (identity).
6. Multipart content-type → 415, not forwarded.
7. Unknown path `/secret` → 404, not forwarded.
8. Store forced unavailable + body with phone → 502, not forwarded.
9. Store unavailable + GET empty → still forwarded.
...
| `/v1/models` | If request also has anthropic headers → Anthropic, else OpenAI Completions | same |

Simpler v1: `/v1/messages`* → Anthropic; `/v1/chat/completions`* and `/v1/models`* and `/v1/embeddings`* → OpenAI Completions; `/v1/responses`* → OpenAI Responses.

Forward:
- Copy method, path, query, all headers except `host` and `accept-encoding` (set identity when body will be scanned or response restored — i.e. all classified routes).
...
Auth: requests without admin token to mutating routes → 401. GET `/api/status` on localhost may use token from header `x-dgw-admin-token` or cookie.
...
- [ ] **Step 2: Run `npm test` — FAIL until pages exist**
...
Pages:
1. Status — process, ports, three upstreams, counts, last error class.
2. Rules — list, enable, priority, regex/dict editor, allowlist, dry-run pane.
3. Upstream & Access — three URLs, bind, optional TLS paths, body limit, TTL, copyable proxy Base URL.
4. Keys & Danger — master key status, rotate form, purge by creator prefix, reset admin token.
5. Live Traffic — streaming table from `GET /api/traffic`.
...
1. `resolveBinary({ env, dataDir, pathExists })` order: `DGW_BIN` → `dataDir/bin/dgw(.exe)` → `PATH`.
...
7. `DGW_NO_DOWNLOAD=1` + missing binary → `{ ok:false, reason: "no_binary" }` without fetching.
...
- [ ] **Step 2: Run `cargo test --workspace` — FAIL if smoke not wired; then implement any glue**
...
| Outbound desensitize, plaintext never upstream | 4–6, 11 |
